...
export const Resolvers = {
  ...
  Subscription: {
    messageAdded: {
      ...
      subscribe: withFilter(
        () => pubsub.asyncIterator(MESSAGE_ADDED_TOPIC), (payload, args) => {
        return Boolean(
          args.groupIds &&
    ...
    groupAdded: {
      subscribe: withFilter(
        () => pubsub.asyncIterator(GROUP_ADDED_TOPIC), (payload, args) => {
          return Boolean(
            args.userId &&
  ...
  Group: {
    ...
    messages(group, { first, last, after, before }) {
      // base query -- get messages from the right group
      const where = { groupId: group.id };

      // Because we return message from newest to oldest
